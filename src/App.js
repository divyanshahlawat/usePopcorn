   ...
   {error && <ErrorMessage message={error}/>}
        ...
        <Box>
          {selectedId?<MovieDetails selectedId={selectedId} onCloseMovie={handleCloseMovie} onAddWatched ={handleAddWatched} watched={watched}/>:<><WatchedSummary watched ={watched}/>
            <WatchedMoviesList watched={watched} onDeleteWatchedMovie={handleWatchedDelete}/></>}
        </Box>
    </Main>
    </>
  );
...
{ const [movie,setMovie] = useState({})
   ...
   const isAlreadyWatched = watched.map((movie)=>movie.imdbID).includes(selectedId)
...
const {Title:title,
      ...
      Poster:poster,
      Runtime:runtime,
      imdbRating,Plot:plot,Released:released,Actors:actors,Director:director,Genre:genre,
} = movie
function addMovie()
{
...
<section>
  <div className="rating">
    {!isAlreadyWatched ? <><StarRating maxRating={10} size={24} onMovieRating={setUserRating}/>
...
function ErrorMessage({message})
{
  ...
  function WatchedMoviesList({watched,onDeleteWatchedMovie})
  {
    return(
      <ul className="list">
                {watched.map((movie) => (
                  <WatchedMovies key={movie.imdbID} movie={movie} onDeleteWatchedMovie={onDeleteWatchedMovie}/>
                ))}
              </ul>
    )
  }
  function WatchedMovies({movie,onDeleteWatchedMovie})
  {
    return(
      <li> 
                    <img src={movie.poster} alt={`${movie.title} poster`} />
